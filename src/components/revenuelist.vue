        ...
        <div class="table-responsive" width="100%">
          <table width="100%">
            <thead width="100%">
              <tr width="100%">
                <th width="20%">
                  <v-select :items=branches v-model="SetBranch" label="Branch:" id="SelBranch" item-text="shortCode" item-value="text"></v-select>
                </th>
                <th width="4%">

                </th>
                <th width="20%">
                  <v-select :items=years v-model="SetYears" label="Years:" id="SelYears" item-text="shortCode" item-value="text"></v-select>
                </th>
                <th width="4%">

                </th>
                <th width="20%">
                  <v-select :items=month v-model="SetMonth" label="Month:" item-text="shortCode" item-value="text" id="SelMonth"></v-select>
                </th>
                <th width="4%">
          ...
          <table v-if="show" width="100%" border="1">
            <thead>
              <tr class="grey lighten-2">
                <th class="text-xs-left" scope="col">Bill Date</th>
                <th class="text-xs-center" scope="col">Amount</th>
                <th class="text-xs-center" scope="col">Entry Date</th>
            ...
            <tbody>
              ...
              <tr scope="row" v-for="(item,index) in revenue_list" :key="index+item.trans_date">
                <td scope="row">{{item.trans_date}}</td>
                <td scope="row" class="text-xs-center">{{item.ftd}}</td>
                <td scope="row" class="text-xs-center">{{item.add_date}}</td>
