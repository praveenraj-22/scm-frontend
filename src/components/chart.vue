  ...
  data: () => ({
   ...
   methods: {
	...
	this.axios
         ...
         .get(`https://scm.dragarwal.com/api-branches/${this.SetEntity}/${selectObj}`).then(response => {
	...
	getRegion(selectObj){
		...
		//	 .get(`https://scm.dragarwal.com/api-region/${selectObj}`).then(response => {
					//console.log(response.data);
	...
	apiRequestCogDashboard(date,SetEntity,SetRegion,SetBranch){
         ...
         .get(`https://scm.dragarwal.com/api-cogs-dashboard/${date}/${entityvalue}/${regionvalue}/${branchvalue}`).then(response => {
