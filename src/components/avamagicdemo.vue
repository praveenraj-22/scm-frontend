        ...
        <v-toolbar flat color="grey lighten-2">
          ...
          <v-menu absolute ref="menu" :close-on-content-click="false" v-model="menu" :nudge-right="40" :return-value.sync="date" lazy transition="scale-transition" offset-y full-width min-width="200px">
            ...
            <v-date-picker color="primary" v-model="date" no-title scrollable min="2018-04-01" :max="today" backgroundRevenue-color="grey" style="box-shadow:none">
              ...
              <v-btn flat color="primary" @click="$refs.menu.save(date);apiRequestUsageTracker(date)" style="outline:none">Generate</v-btn>
            </v-date-picker>
          ...
          <download-excel :data="json_data" :fields="json_fields" type="csv" :name="fileName" :title="xlTitle" :fetch="downloadExcelUsageTraker">
        ...
        <div class="table-responsive">
          <table class="table table-hover table-bordered" v-if="show">
            ...
            <tr class="grey lighten-2">
              <td class="text-xs-left" width="10%" scope="col"></td>
              <td width="10%" class="text-xs-center" scope="col"></td>
              <td width="10%" class="text-xs-center" scope="col"></td>
              <td width="14%" class="text-xs-center" scope="col" colspan="2"><b>New OPD</b></td>

              <td width="35%" class="text-xs-center" scope="col" colspan="5"><b>Perimeter (Paid Count)</b></td>

              <td width="14%" class="text-xs-center" scope="col" colspan="3"><b>Perimeter Revenue (INR) </b></td>

            </tr>


            <tr class="grey lighten-2">
              <td class="text-xs-center" width="10%" scope="col"><b>Entity</b></td>
              <td width="10%" class="text-xs-center" scope="col"><b>Region</b></td>
              <td width="10%" class="text-xs-center" scope="col"><b>Branch</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>FTD</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>MTD</b></td>

              <td width="7%" class="text-xs-center" scope="col"><b>FTD</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>MTD</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>Target</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>Tar Ach%</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>FTD</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>MTD</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>Target Amount</b></td>
              <td width="7%" class="text-xs-center" scope="col"><b>Tar Ach%</b></td>
            </tr>
            ...
            <tbody>
              ...
              <tr scope="row" v-for="(item,index) in group" :key="index+item.branch" class="grey lighten-4">
                <td width="10%" scope="row" class="text-xs-center branchesgrp1">Total</td>
                <td width="10%" scope="row" class="text-xs-center branchesgrp1"></td>
                <td width="10%" scope="row" class="text-xs-center branchesgrp1"></td>
                <td width="7%" scope="row" class="text-xs-center rotgrp1">{{item.ftdopdrev}}</td>
                <td width="7%" scope="row" class="text-xs-center rotgrp1">{{item.mtdopdrev}}</td>

                <td width="7%" scope="row" class="text-xs-center allindiagroup1">{{item.revenueftdcount}}</td>
                <td width="7%" scope="row" class="text-xs-center allindiagroup1">{{item.revenuemtdcount}}</td>
                <td width="7%" scope="row" class="text-xs-center allindiagroup1">{{item.target}}</td>
                <td scope="row" class="text-xs-center allindiagroup1">{{item.revenuetargetachived}}</td>
                <td width="7%" scope="row" class="text-xs-center ochfont1">{{item.revenueftdamount}}</td>
                <td width="7%" scope="row" class="text-xs-center ochfont1">{{item.revenuemtdamount}}</td>
                <td width="7%" scope="row" class="text-xs-center ochfont1">{{item.targetamount}}</td>
                <td width="7%" scope="row" class="text-xs-center ochfont1">{{item.targetamountach}}</td>

              </tr>

              <tr scope="row" class="grey lighten-4">
                <td width="10%" scope="row" class="text-xs-center branchesgrp1"></td>
                <td width="10%" scope="row" class="text-xs-center branchesgrp1"></td>
                <td width="10%" scope="row" class="text-xs-center branchesgrp1"></td>
                <td width="7%" scope="row" class="text-xs-center rotgrp1"></td>
                <td width="7%" scope="row" class="text-xs-center rotgrp1"></td>

                <td width="7%" scope="row" class="text-xs-center allindiagroup1"></td>
                <td width="7%" scope="row" class="text-xs-center allindiagroup1"></td>
                <td width="7%" scope="row" class="text-xs-center allindiagroup1"></td>
                <td scope="row" class="text-xs-center allindiagroup1"></td>
                <td width="7%" scope="row" class="text-xs-center ochfont1"></td>
                <td width="7%" scope="row" class="text-xs-center ochfont1"></td>
                <td width="7%" scope="row" class="text-xs-center ochfont1"></td>
                <td width="7%" scope="row" class="text-xs-center ochfont1"></td>
              ...
              <tr scope="row" v-for="(item,index) in sortbranches" class="grey lighten-4">
                <td width="10%" scope="row" class="text-xs-center branchesgrp1">{{item.entity}}</td>
                <td width="10%" scope="row" class="text-xs-center branchesgrp1">{{item.region}}</td>
                <td width="10%" scope="row" class="text-xs-center branchesgrp1">{{item.branch}}</td>
                <td width="7%" scope="row" class="text-xs-center rotgrp1">{{item.ftdopdrev}}</td>
                <td width="7%" scope="row" class="text-xs-center rotgrp1">{{item.mtdopdrev}}</td>
                ...
                <td width="7%" scope="row" class="text-xs-center allindiagroup1">{{item.revenuemtdcount}}</td>
                <td width="7%" scope="row" class="text-xs-center allindiagroup1">{{item.target}}</td>
                <td width="7%" scope="row" class="text-xs-center allindiagroup1">{{item.revenuetargetachived}}</td>
                <td width="7%" scope="row" class="text-xs-center ochfont1">{{item.revenueftdamount}}</td>
                <td width="7%" scope="row" class="text-xs-center ochfont1">{{item.revenuemtdamount}}</td>
                <td width="7%" scope="row" class="text-xs-center ochfont1">{{item.targetamount}}</td>
                <td width="7%" scope="row" class="text-xs-center ochfont1">{{item.targetamountach}}</td>
