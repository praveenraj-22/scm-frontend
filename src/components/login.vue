  ...
  methods: {
    submit () {
	  ...
	  console.log("11111111111");
        this.loading = true;
        this.axios
          .post(`https://scm.dragarwal.com/login`, {
          ...
          .then(response => {
            ...
            if (
              ...
              sessionStorage.setItem("super_user", JSON.stringify({ name: this.name, userName: response.data.userName,role : response.data.role }));
              ...
              response.data.isAuthenticated === true &&
              response.data.role === "indian_user"
            ) {
              sessionStorage.setItem("indian_user", JSON.stringify({ name: this.name, userName: response.data.userName,role : response.data.role }));
              serverBus.$emit("changeComponent", "revenueSuper");
              // serverBus.$emit("changeComponent", "chart");
            }else if(
              response.data.isAuthenticated === true &&
