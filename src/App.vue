...
export default {
  ...
  created () {
   ...
   this.userType = 'optical'
	  ...
	  if(this.userType=='super' && this.userId!=103292){
		if(this.userId==104860){
			this.tabItems = ['Revenue', 'Cogs Vs Revn','Dashboard','NewOPD']
		}else if(this.userType=='super' && (this.userId==104038 || this.userId==102170)) {
 		  this.tabItems = ['Revenue','Cogs Vs Revn','Dashboard','NewOPD','Optical','Discount']
		}else{
			this.tabItems = ['Revenue', 'Cogs Vs Revn','Dashboard','NewOPD','Optical']
		}
	  }
	  else if(this.userType=='super' && this.userId==103292) {
		  this.tabItems = ['Revenue', 'Cogs Vs Revn','Dashboard','Surgery','NewOPD','Optical','Discount']
	  }
  ...
  methods: {
   ...
   serverBus.$emit('changeComponent', 'Discount')
    ...
    if ((sessionStorage.getItem('super_user')) || (sessionStorage.getItem('optical_user')) || (sessionStorage.getItem('indian_user'))) {
